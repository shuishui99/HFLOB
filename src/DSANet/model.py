...
from DSANet.dsanet.Layers import EncoderLayer, DecoderLayer
...
class DSANet(LightningModule):
    ...
    def loss(self, labels, predictions):
        if self.params.criterion == 'l1_loss':
            loss = F.l1_loss(predictions, labels)
        elif self.params.criterion == 'mse_loss':
    ...
    def __dataloader(self, train):
        ...
        set_type = train
        dataset = MTSFDataset(window=self.params.window, horizon=self.params.horizon, data_name=self.params.data_name, set_type=set_type, data_dir=self.params.data_dir)
    ...
    def add_model_specific_args(parent_parser, root_dir): # pragma: no cover
        ...
        parser.add_argument('--data_name', type=str, default='exchange_rate.txt')  # add by jam
        parser.add_argument('--data_dir', default='./data', type=str)
        # num of univariate time series
        parser.add_argument('--n_multiv', type=int, default=8)  # add by jam
